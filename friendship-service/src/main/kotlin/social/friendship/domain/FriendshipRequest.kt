...
class FriendshipRequest private constructor(val to: User, val from: User) : AggregateRoot<FriendshipRequestID>(FriendshipRequestID(to.id, from.id)) {
    /**
     * Data class to represent the friendship request ID.
     */
    data class FriendshipRequestID(val to: UserID, val from: UserID) : ID<Pair<UserID, UserID>>(Pair(to, from))
